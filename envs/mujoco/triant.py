## This version is to train single agent with velocity. No wining rate, no [0]
import numpy as np
...
class TriantEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        ...
        if self.spider_pos[1:2] >= self.bug_pos[1:2]:
            ...
            y_reward = - self.bug_vel[1:2]- self.spider_vel[1:2]    #- self.spider_vel[1:2] +?
        ...
        reward = 5 * self_opponent_reward \
            + 5* partner_reward \
                -4       
        

        """
        #short: spider attacks ant
        #x
        # ant >= spider
        if self.ant_pos[0:1] >= self.spider_pos[0:1]:
        ...
        reward = 5 * x_reward + 5* y_reward 
        """
        
        """
        #long: ant attacks spider
        #x
        # spider >= ant
        ...
        reward = 5 * x_reward + 5* y_reward -4 
        #\- 2*(self.ant_pos[1:2] - self.spider_pos[1:2])**2 - 2*(self.ant_pos[0:1] - self.spider_pos[0:1])**2 - 4   ## train with velocity for middle agent     
        """
        ...
        """
        # Short: bug attacks ant
        #x
        # ant >= bug
        if self.ant_pos[0:1] >= self.bug_pos[0:1]:
            x_reward = self.ant_vel[0:1] + self.bug_vel[0:1] 
        ...
        # y       
        # ant >= bug
        if self.ant_pos[1:2] >= self.bug_pos[1:2]:
            y_reward = self.ant_vel[1:2] + self.bug_vel[1:2] 
        ...
        # ant >= spider
        if self.ant_pos[0:1] >= self.spider_pos[0:1]:
            partner_x_reward = self.spider_vel[0:1] 
        ...
        # ant >= spider 
        if self.ant_pos[1:2] >= self.spider_pos[1:2]:
            partner_y_reward = self.spider_vel[1:2] 
        ...
        reward = 5 * x_reward + 5* y_reward \
            + 3 * partner_x_reward + 3 * partner_y_reward \
              - 4
        """
        ...
        return ob, reward, done, {} #sparse_reward
