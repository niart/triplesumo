## This is for synchronous training of ant and bug
import numpy as np
...
class Ant_bug_triantEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        ...
        if self.spider_pos[1:2] >= 0:
            ...
            y_reward = - self.spider_vel[1:2]            
        reward = 5 * x_reward + 5* y_reward - 2*(self.ant_pos[1:2] - self.spider_pos[1:2])**2 - 2*(self.ant_pos[0:1] - self.spider_pos[0:1])**2 - 2*(self.bug_pos[1:2] - self.spider_pos[1:2])**2 - 2*(self.bug_pos[0:1] - self.spider_pos[0:1])**2 - 4
