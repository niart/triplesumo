## This is to train individual agent, a 'winning rate' returned, has [0]
import numpy as np
...
class TriantEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        ...
        reward = 5 * x_reward + 5* y_reward \
            - 2*(self.ant_pos[1:2] - self.spider_pos[1:2])**2 - 2*(self.ant_pos[0:1] - self.spider_pos[0:1])**2 \
                - 4 ## train with velocity for big agent; +500 if ant dies; -400 if spider dies 
        ...
        if self.spider_pos[1:2] >= 0:
            ...
            y_reward = - self.spider_vel[1:2]            
        reward = 5 * x_reward + 5* y_reward \
            - 2*(self.bug_pos[1:2] - self.spider_pos[1:2])**2 - 2*(self.bug_pos[0:1] - self.spider_pos[0:1])**2 \
                - 4
        ...
        if done:        
            if self.isout(self.bug_pos) or self.isout(self.ant_pos):
                reward -= 2000
                self.score = -1
                print("The team dies !!!!!!!!!!!!!!!!!!!!!!")      
